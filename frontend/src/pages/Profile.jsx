      ...
      try {
        ...
        window.contributions = await Promise.all(
          projects.filter(p => +p.creator !== +userAddress)
            .map((p, i) => {
              const qavah = new ethers.Contract(qavahAddress.Address, Qavah.abi, window.provider)
              return p.donators.map((d) => +d === +userAddress &&
                qavah.tokenURI(i).then(q => JSON.parse(atob(q.split(',')[1])))
              ).filter(Boolean).reverse()
            }).flat()
        )
        console.log("Window contributions: ", window.contributions)
